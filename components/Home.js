    ...
    .then(data => {
      ...
      data.movies.forEach(element => {
          ...
          if (element.overview.length > 250) {
            element.overview = `${element.overview.slice(0, 249)}...`
          } 
          formatedData.push({title: element.title, poster: `https://image.tmdb.org/t/p/w500/${element.backdrop_path}`, voteAverage: element.vote_average, voteCount: element.vote_count, overview: element.overview})
        })
